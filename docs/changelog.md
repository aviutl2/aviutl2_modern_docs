...
- 幾つかのシーン選択メニューでラベルの階層を表示するように修正
- バッチ出力時にプロジェクトデータのファイル出力設定が反映されないのを修正
- 画面の設定に高パフォーマンスGPUを優先するかの設定項目を追加
- ノイズフィルタに渦度の設定項目を追加
- ノイズフィルタのノイズの種類を追加
- ノイズフィルタの生成処理を改善
- 揺らぎフィルタのスクリプトを追加
...
- グリッド（BPM）の設定でオフセット値がある時に不要なグリッドが表示されるのを修正
- クロマキー、カラーキー、特定色域変換の境界補正の処理を修正
- テキスト編集の対象をオブジェクトの全てのテキスト項目を対象とするようにした
- ディスプレイスメントマップに領域外を境界色とする設定を追加
- ノイズフィルタの合成モード項目の種類を追加
- ノイズ（仮想バッファ）のフィルタ効果を追加
- プロジェクトの読み込みエラー時の表示を少し変更
- パーティクルの初期ラベルを変更

## v2.1.8 <Badge text='2026/9/5' />

- シーンリストの表示対象ラベルの情報をプロジェクトに保持するように修正
- 設定ウィンドウのスクリプト等に複数のファイル設定項目がある場合のD&Dに対応
- 色設定のタイトルバーに対象の色設定項目のアイコンを追加
- 画像合成のフィルタ効果に配置の設定項目を追加
- パーティクル出力で出力方向・回転角の基準を移動方向にした時の動作を修正
- パーティクル出力の生存時間に応じて透過・縮小するタイミングを調整するようにした
- パーティクル出力（カメラ制御）を仮追加
...
- クリッピングオブジェクトの対象がグループ制御の場合に配下のグループ制御の範囲も対象となるように修正
- トラックバーの移動方法を変更した時に設定値を集約する場合の処理を調整
- シーンの設定情報にラベルの項目を追加
- シーンリストの一覧に表示するシーンの対象ラベルを選択する機能を追加
- シーンのプリセットを作成出来るようにした
- 幾つかの文言を調整

## v2.1.6a <Badge text='2026/8/22' />

- グラデーション、グラデーションマップで存在しない仮想バッファをパターン画像に指定した時に落ちるのを修正

## v2.1.6 <Badge text='2026/8/22' />

- シーンの移動時に条件によって落ちることがあるのを修正
- オブジェクト設定でグループ項目が2重に表示されることがあるのを修正
- プレビュー編集画面の<kbd>Alt</kbd>+ドラッグのリサイズ操作でサイズ項目のUndoが反映されないのを修正
- トラックバーの移動方法の設定ダイアログの一部項目で言語設定が正しく反映されないのを修正
- 言語ファイルのツールチップ項目が足りない場合にDefault言語の差分ファイルも参照するように修正
- 設定ファイル`aviutl2.ini`の更新を一時ファイルを作成して置き換える方法に修正
- ウィンドウを分離した場合の親ウィンドウのIMEの初期状態を無効とするように修正
- シーン再生でレイヤー編集の対象シーンをシーン再生と同じシーンに移動する操作を追加
- 色調補正、拡張色調補正で輝度・彩度・色相補正時の色空間を変更出来るようにした
- グラデーションフィルタに色補間、パターン画像の設定を追加
- グラデーションマップを仮追加

## v2.1.5 <Badge text='2026/8/16' />

- アプリケーションのGPU設定が自動の場合にパフォーマンス重視のGPUを優先するようにした
- オブジェクト設定のスライダー項目の表示範囲と最大範囲が異なる場合のスライドの移動量を調整
- レイヤー編集のマーク、グリッド（BPM）の開始位置のダブルクリックで設定ダイアログを表示するようにした
- オブジェクト設定のサブグループ項目の表示オプションの対象を明示的なグループ項目以外とするようにした
- オブジェクト設定のオブジェクト毎のスクロール位置を維持するようにした
- 時間・フレーム数を設定するダイアログの入力状態を維持するようにした
- プレビュー再生時にウィンドウを前面に表示して再生するようにした
- ウィンドウメニューの編集にオブジェクト・フィルタ効果の追加、プリセット反映の操作を追加
- レイヤー編集のメニューにフィルタ効果の追加、プリセット反映の操作を追加
- メディアオブジェクトに上のオブジェクトでクリッピングする機能を追加
- パーティクル出力の出力頻度の上限値とスライダーの表示範囲を調整
- 色設定でカラーコードの表示と入力を出来るようにした
- オブジェクト設定の色設定項目の表示と入力文字制限を調整
- 色調補正の色相の設定項目のスライダーの表示範囲を調整
- 色調補正で輝度が小さい時の彩度の計算を調整
- 色調補正にガンマの設定項目を追加
- 拡張色調補正を仮追加
