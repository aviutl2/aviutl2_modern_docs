...
- 汎用データ領域の保存データを初期化する条件を変更
- `--hide`の非表示対象に指定出来る設定項目と特殊条件を追加
...
- トラックバー移動スクリプトで`--require`の定義が反映されないのを修正
...
- トラックバー移動スクリプトの`--param`にチェックボックスやリスト選択を作成出来るようにした
- `obj.load("figure")`にアスペクト比の引数を追加

## 2.1.1 <Badge text='2026/7/18' />

- `print()`で循環参照したテーブルを出力した時に落ちるのを修正
- `--dialog`の色指定で初期値が`nil`の場合に正しく反映されないのを修正 ※互換対応
- `--dialog`の項目名が先に定義された項目名と重複している場合に項目名を調整するようにした ※互換対応
...
- `obj.getinfo("bpm_list")`を追加
- `obj.getoption()`に`"blend"`、`"culling"`、`"billboard"`を追加
- `obj.getfont()`を追加
...
- `global`テーブルの変数をバイナリセーフな文字列として保持するようにした
- `obj.getvalue(layer,effect,item,...)`を追加
