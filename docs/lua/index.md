...
※項目名を`yyy::xxx`のように指定するとxxxの部分のみ設定画面で表示される形に出来ます。
...
※0.0001以下も指定出来ますが最大最小値の範囲に応じて調整されます。（内部は32bit整数で保持されます）
...
スクリプトファイルの先頭で`--checksection@変数名:項目名,デフォルト値(trueかfalse)`のように指定するとセクション毎のチェックボックスが有効になります。
