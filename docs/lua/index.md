...
※項目名はスクリプト毎にユニークにする必要があります。（設定値が保存される項目のみ）
...
※旧スクリプトファイル形式の`--track0:項目名,最小値,最大値,デフォルト値,移動単位`も利用できます。（互換対応）
...
※旧スクリプトファイル形式の`--check0:項目名,デフォルト値（0か1）`も利用できます。（変数はboolean型）（互換対応）
...
※旧スクリプトファイル形式の`--color:デフォルト値`も利用できます。（互換対応）
...
※旧スクリプトファイル形式の`--file:`も利用できます。（互換対応）
...
スクリプトファイルの先頭で`--data@登録名:サイズ（16KB以下）`のように指定すると汎用データ領域が有効になります。※スクリプトモジュールやDLL向けになります。
...
スクリプトファイルの先頭で`--hide@変数名:条件`のように指定すると、条件を満たした場合に変数名の項目が非表示になります。※条件が未指定の場合は常に非表示になります。
条件は`(変数名)(比較演算子)(数値)`の形で指定します。

非表示対象に指定出来る設定項目の種別：

- トラックバー項目、チェックボックス項目等の設定値を持つ項目

非表示条件に指定出来る設定項目の種別、または特殊条件：

- チェックボックス項目
- リスト選択項目
- ファイル選択項目
- フォルダ選択項目
- `filter`：フィルタオブジェクトか？（`0`/`1`）

※セクション毎のチェックボックスはセクション毎が有効の場合は`2`を返却（`0`/`1`/`2`）
※ファイル、フォルダ選択項目は選択されているかを返却（`0`/`1`）

非表示条件の比較演算子：`==`、`~=`、`>`、`<`

```aulua
--track@pos:位置,-100000,100000,0
--check@chk:位置を利用,0
--value@poslist:座標,{}
--hide@pos:chk==0
--hide@pos:filter==1
...
※旧スクリプトファイル形式の`--dialog`、`--param`も利用できます。個々の設定項目が作成されます。（互換対応）
...
| `obj.aspect`<sup>1、2</sup> | アスペクト比（-1.0～1.0/プラス=横縮小/マイナス=縦縮小）                                                        |          |
...
| `obj.frame_s`               | 全体（シーン）基準のオブジェクトの開始フレーム（0からの整数）                                                  | ○        |
| `obj.frame_e`               | 全体（シーン）基準のオブジェクトの終了フレーム（0からの整数）                                                  | ○        |
| `obj.effect_layer`          | 対象エフェクトが配置されているレイヤー ※自身のオブジェクトのレイヤー位置                                       | ○        |
| `obj.originframe`           | 全体（シーン）基準のレンダリングの起点フレーム（0からの整数）                                                  | ○        |
...
- `aspect`：アスペクト比（-1.0～1.0/プラス=横縮小/マイナス=縦縮小）
- 戻り値：`true`=成功 / `false`=読み込み失敗
...
- `value`：0=向かない / 1=横方向のみ / 2=縦横方向のみ / 3=向く
...
- 戻り値：0=カメラ制御対象外 / 1=カメラ制御対象 / 2=カメラ制御対象（編集用視点）
...
- 戻り値：0=向かない / 1=横方向のみ / 2=縦横方向のみ / 3=向く
...
- `target`：設定種別
  ...
  - `"aspect"`：基準アスペクト比（-1.0～1.0/プラス=横縮小/マイナス=縦縮小）<sup>1、2</sup> ※互換対応
...
- `option`：各種オプションを列挙出来ます。
  ...
  - `{座標テーブル}`：`name`で指定されている座標の数が正しくない場合に指定の座標テーブルでアンカーを設定します。
    ※`name`で定義されている項目への反映はアンカーが移動した時になります。
- 戻り値：取得したアンカーポイントの数
...
Lua標準の`require()`の引数をUTF-8文字列で利用出来るようにフックしています。  
※Lua内部はSJIS扱いとなっているので`package.xxx`関係のパスはSJISで格納されます。
