...
オブジェクト自体を<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+ドラック操作で中心座標を移動することが出来ます。
オブジェクト自体を<kbd>Alt</kbd>+ドラック操作で拡大率を変更することも出来ます。
※入力設定に「サイズ」項目が存在して「移動無し」の場合は拡大率ではなくサイズを調整します。
拡大縮小アンカーは<kbd>Alt</kbd>+ドラック操作でZ軸回転の設定を変更することも出来ます。
...
<kbd>Alt</kbd>キーを押しながらダブルクリックするとレイヤー編集をシーン再生と同じシーンに移動することが出来ます。
<kbd>Ctrl</kbd>+マウスホイール操作でマウス位置を基準にプレビュー画面の拡大率を変更出来ます。
...
シーンの設定やシーン名の変更からラベルを設定することが出来ます。
※ラベル名に`\`で区切られた文字を指定すると複数階層にすることが出来ます。

上部のタイトルバーから一覧に表示するシーンの対象ラベルを選択することが出来ます。
上部のアイコンからシーンの追加や現在のシーンを削除することが出来ます。
...
マークされたフレームの一覧を表示します。  
マークされたフレームはレイヤー編集ウィンドウで表示されるようになります。  
レイヤー編集ウィンドウ上部の右クリックメニューからフレームのマークやメモの変更、ドラッグ操作による設定フレームの移動をすることが出来ます。
...
中間点と制御点をリンクする設定を有効にすると中間点と制御点の時間が同期するように自動配置されます。  
※リンクしている場合は制御点ではなく中間点の移動や追加削除の操作になります。  
※リンクしている場合に<kbd>Shift</kbd>キーを押しながら中間点を移動すると選択点以前以降が移動します。

<kbd>Ctrl</kbd>+マウスホイール操作で編集画面の拡大率を変更出来ます。  
※横方向のみ拡大するかを設定することが出来ます。  
右ボタンドラッグで編集画面をスクロール出来ます。  
右クリックメニューから各種操作が出来ます。

コピー、貼り付け操作は時間制御設定の全体が対象になります。  
上部のタイトルバーから対象の設定項目を選択出来ます。
上部のリストアイコンからプリセットの読み込み、作成をすることが出来ます。

## テキスト編集

選択中オブジェクトのテキストを編集します。  
オブジェクトにテキスト項目が含まれる場合に編集することが出来ます。
...
VRAMリソースキャッシュはリソース領域を再利用する為のキャッシュになります。（0に設定した場合は無効になります）
※フレーム毎に利用するリソース量とGPUメモリ量に適したキャッシュを設定してください。
※VRAMリソースキャッシュはデータのキャッシュとは異なりキャッシュ破棄操作の対象外となります。
VRAMデータキャッシュは静止画のみキャッシュ対象となります。（0に設定した場合は無効になります）

## コーデックの設定

ウィンドウメニューの「設定」→「コーデックの設定」から標準のAVIファイル入出力利用時のコーデックのYUV圧縮展開の設定を変更することが出来ます。

## ファイル出力の設定

ウィンドウメニューの「設定」→「ファイル出力の設定」からファイル出力完了時のサウンドやYUV出力変換の設定を変更することが出来ます。
...
ウィンドウメニューの「設定」→「画面の設定」から高DPI表示、高パフォーマンスGPUを優先する設定を変更することが出来ます。  
※アプリケーションの再起動後に反映されます。  
※高DPI環境で拡大サイズ表示した場合に見えづらくなる場合があります。  
※高パフォーマンスGPUを優先しない場合はシステムの既定のGPUが利用されます。
...
入力がYUV形式の場合はYUVの色空間の設定をすることが出来ます。  
入力に音声がある場合はフレームレートをオーディオの長さに合わせるように調整する設定が利用出来ます。  
入力が可変フレームレートに対応している場合は強制的にフレームレートを固定する設定が利用出来ます。  
※ファイル内容によっては期待したフレーム位置にならない場合があります。
...
※シークや再生時のコマ落ち、シーンの参照等でオブジェクトが連続して表示されない時は正しい表示になりません。
...
明るさ、色あい等を調節します。
飽和するを設定すると補正後にRGB値を標準の範囲に収めます。
色空間の設定で輝度・彩度・色相補正時の色空間を指定することが出来ます。

## グラデーション・グラデーションマップ

画像をグラデーションさせます。
色を指定した場合は色補間の設定から補間する色空間を指定することが出来ます。
パターン画像を指定した場合は画像をグラデーションの色（カラーバー）として利用します。
※シーン等で複数色を利用したカラーバーを作成して参照することが出来ます。
...
上位のレイヤーにあるメディアオブジェクトでクリッピングをしてから描画するようにします。
※クリッピングオブジェクトのようなマスクの合成やグループ制御やカメラ制御へのクリッピングは出来ません。
※オブジェクトの種類や状態によっては反映されない場合があります。

## 参照式

オブジェクト設定の数値項目の名前のクリックメニューから参照式を設定することが出来ます。  
参照式を設定すると自身やその他の項目の値を参照して計算した結果を最終的な値とすることが出来ます。
※項目によっては参照式が反映されない場合があります。（設定値から独自に値を算出している場合等）
